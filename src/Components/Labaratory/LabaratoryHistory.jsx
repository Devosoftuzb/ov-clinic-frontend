...
function LabaratoryHistory() {
  ...
  let getDataFromActive = "input value" // active page table input value submit and get it

  // logic for 24hours and input or li with text
  let postTime = "2024-09-13T16:28:00Z";
  const [is24HoursPassed, setIs24HoursPassed] = useState(null);
  useEffect(() => {
    const now = new Date();
    const postedTime = new Date(postTime)
    // const localPostedTime = new Date(postedTime.getTime() - (postedTime.getTimezoneOffset() * 60000));
    const timeDifference = (now - postedTime) / (1000*60*60);  
      
    // const twentyFourHours = 24 * 60 * 60 * 1000;
    if (timeDifference >= 19) {
      setIs24HoursPassed(true)
    } else {
      setIs24HoursPassed(false)
    }
  }, [postTime]);
  let calc_length_array_sers_patent = 7
  let filter_number_array_length_checked = 7
          ...
          <div className='Kassa__main__table'>
            <table>
              ...
              Посмотрено
                        ...
                        <h3 className='setmoney status_lab_check'>
                          Осмотрено
                        </h3>
                      </td>
                      <td>
        ...
        <div>
          <div className='Kassa__info__header'>
            ...
            <button className='Kass__info__header__btn' onClick={(e) => openPatientTable(e)}>
              <svg className='Register__main__out__btn2__svg2' xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 48 48"><path fillRule="evenodd" strokeLinejoin="round" width={4} d="M44 40.836c-4.893-5.973-9.238-9.362-13.036-10.168c-3.797-.805-7.412-.927-10.846-.365V41L4 23.545L20.118 7v10.167c6.349.05 11.746 2.328 16.192 6.833c4.445 4.505 7.009 10.117 7.69 16.836Z" clipRule="evenodd"></path></svg>
          ...
          <div className='Kassa__info doctor__info'>
            ...
            <div className='Kassa__info__grid'>
              ...
              <span className={`Kassa__info__grid__money labaratory_patient_status ${box_width === 0 ? "box_border_red" : box_width === 100 ? "box_text_color" : ""}`}>
                ...
                {foundService.tbody.map((item, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td>
                        {is24HoursPassed ? (
                          <li>{getDataFromActive}</li>

                        ) : (

                          <input defaultValue={getDataFromActive} type="text" name="" id="" />
                        )}
                      </td>
                    </tr>
                  )
...
export default LabaratoryHistory